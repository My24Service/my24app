  ...
  Widget _buildMaterialsTable() {
    ...
    rows.add(TableRow(
      children: [
        ...
        Column(children: [
          createTableHeaderCell('assigned_orders.materials.info_location'.tr())
        ]),
        Column(children: [
    ...
    for (int i = 0; i < materials.results.length; ++i) {
      ...
      if (material.location == null || material.material == null) {
        continue;
      }

      rows.add(TableRow(children: [
        Column(children: [createTableColumnCell('${material.materialName}')]),
        Column(children: [createTableColumnCell('${material.locationName}')]),
        ...
        TypeAheadFormField(
          textFieldConfiguration: TextFieldConfiguration(
              ...
              decoration: InputDecoration(
                  labelText:
                  'assigned_orders.materials.typeahead_label_search_material'.tr())),
          suggestionsCallback: (pattern) async {
            if (pattern.length < 1) return null;
            return await inventoryApi.searchLocationProducts(_locationId, pattern);
          },
